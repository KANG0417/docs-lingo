...
영문 기술 문서를 한국어로 번역해 주는 서비스입니다. 다만 단순한 번역기를 만들고 싶지는 않았습니다.

요즘은 브라우저 자체 번역으로도 페이지 전체를 읽을 수 있습니다. 그런데 막상 공식 문서를 번역해서 읽어 보면, 어디가 중요한지 모른 채 긴 글을 처음부터 끝까지 따라가게 됩니다. 독스링고는 그 지점을 해결하려는 도구입니다. 문서를 통째로 옮기는 대신 **현업에서 실제로 쓰이는 내용 위주로 정제해 번역**하고, 함께 공부하면 좋은 **핵심 키워드**를 뽑아서 보여 줍니다. 번역문을 읽는 동시에 "이 문서에서 뭘 가져가야 하는지"가 눈에 들어오도록 만드는 것이 목표입니다.

공식 문서 URL을 넣거나 텍스트를 직접 붙여 넣으면, 본문 구조를 그대로 유지한 번역과 키워드 요약을 받아볼 수 있습니다. 더 공부하고 싶은 문서는 북마크에 모아 두고, 최근에 본 문서는 히스토리에서 다시 꺼내 볼 수 있습니다.

---

## 프로젝트 로드맵

2026년 6월 10일에 시작해 현재 1단계(MVP)를 개발하고 있습니다.
